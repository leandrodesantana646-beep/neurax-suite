...
# Função Inteligente Híbrida (Usa OpenAI se tiver chave, senão usa IA simulada avançada)
def processar_ia(tipo, dados):
    # Tenta usar OpenAI se a chave foi informada
    if client:
        try:
            if tipo == "copy":
    ...
    # Modo Inteligente Padrão (Funciona sem chave de API!)
    if tipo == "copy":
        p = dados['produto']
        pub = dados['publico']
